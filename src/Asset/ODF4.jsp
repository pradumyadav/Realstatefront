		...
		<div id="order_h" style="display:flex;justify-content: space-between;align-items: center;">
			...
			<div style="position: relative;z-index: -1;border: 1px solid #dfdfdf;margin: 0px 10px;border-radius: 3px;">
				...
				<div class="BookStock" id="BookStock">
					...
					<div class="Order_items_h1" style="display: none;">
					...
					<div class="text-center mt-1" style="display: flex;justify-content: space-around; align-items: center; margin-top: 30px;">
	...
	$.ajax({
		  ...
		  url: window.baseurl+"getFunction.do?actn=getsqljsondata&sqlfieldid=63264&ids=AID/random/&"+"valuestring="+AID+"@j@"+random+"@j@&cloudcode=ceramicdemo&type=json"											
			,   success: function(result){
				...
				var selected_cid =  $("#CID").val()==""?0: $("#CID").val();;
	        	for(j=0;j<jsonresult.length;j++){
	        		data=jsonresult[j];
	        		var oddoreven = j%2?'oddrow':'evenrow';
					var seleccted = selected_cid == data.CID?"selected":"";
	        		...
	        		var field = '<tr class="'+oddoreven+' '+seleccted+'" >'+
...
function create_odf_table(){
	...
	console.log(result);
	const jsonresult = result;
	var header ='<table class="Pending_order_items_stock_table" id="Pending_order_items_stock_table" style="width:1450px;margin-top:20px">'+
	'<thead>'+
	...
	for(j=0;j<jsonresult.length;j++){
		...
		tablebody += field;                        
			...
			if(jsonresult.length < 4){
						tablebody =(jsonresult.length == 0 ? tablebody +="<tr></tr><tr style='text-align:center;'>Stock is not Available</tr><tr></tr><tr></tr>":tablebody); 
						tablebody =(jsonresult.length == 1 ? tablebody +="<tr></tr><tr></tr><tr></tr><tr></tr><tr></tr><tr></tr>":tablebody); 
						tablebody =(jsonresult.length == 2 ? tablebody +="<tr></tr><tr></tr><tr></tr><tr></tr>":tablebody); 
						tablebody =(jsonresult.length == 3 ? tablebody +="<tr></tr>":tablebody); 
				}
		...
		if(jsonresult.length>0){
			...
			document.getElementById("loadContent").innerHTML='<div class="center-content"><h2>Stock Is Not Available</h2></div>';
			balance_order_stock();
		}
			balance_order_stock();
...
function balance_order_stock(){
	...
	var random = genraterandom(5);
		...
		var seprator="@j@";
		var valuestring = encodeURI(NAME_OF_COMPANY)+seprator+encodeURI(BRANCH_NAME)+seprator+encodeURI(ITEM_CODE)+seprator+encodeURI(QUALITY)+seprator+encodeURI(BRAND)+seprator+encodeURI(PACKING_CODE)+seprator+PACKING_QTY+seprator+BID+seprator+CID+seprator+BOOK_STOCK_FILTER+seprator+random+seprator;
    	$.ajax({
		  	type: "GET",
		  	url: window.baseurl+"getFunction.do?actn=getsqljsondata&sqlfieldid=63268&ids=NAME_OF_COMPANY/BRANCH_NAME/ITEM_CODE/QUALITY/BRAND/PACKING_CODE/PACKING_QTY/BID/CID/PACKING_TYPE/random/&valuestring="+valuestring+"@j@&cloudcode=ceramicdemo&type=json"													
			,success: function(result){	
